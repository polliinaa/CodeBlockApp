            ...
            try {
                ...
                when (block) {
                    is DeclareVariableBlockData -> {
                        try {
                            val value = evaluator.evaluate(block.value)
                            ...
                            printOutput.add("Error declaring variables '${block.name}': ${e.message}")
                        }
                    }
                    ...
                    is AssignVariableBlockData -> {
                        try {
                            ...
                            val targetContext = if (parentIf != null) {
                                if (evaluator.currentContext().containsKey(block.selectedVariable)) {
                                    ...
                                    evaluator.getGlobalContext()
                                } else {
                                    printOutput.add("Error: Variable '${block.selectedVariable}' not found")
                                    null
                                }
                            } else {
                            ...
                            targetContext?.put(block.selectedVariable, result)
                        } catch (e: Exception) {
                            printOutput.add("Error assigning variable '${block.selectedVariable}': ${e.message}")
                        }
                    }
                    ...
                    is FunctionPrintBlockData -> {
                        try {
                            ...
                            printOutput.add("Print error: ${e.message ?: "Invalid expression '${block.expression}'"}")
                        }
                    }
                }
            } catch (e: Exception) {
                printOutput.add("Execution error in block ${block::class.simpleName}: ${e.message}")
            }
        }
